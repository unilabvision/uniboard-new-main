        ...
        <div className="absolute bottom-3 right-3 menu-container">
          ...
          {showMenu && (
            <div className="absolute right-0 bottom-full mb-2 w-40 bg-white dark:bg-neutral-800 rounded-md border border-neutral-200 dark:border-neutral-700 shadow-lg z-10">
              <Link
                href={`/lms-2/courses/${course.slug}`}
...
export default function LMSPage({ searchParams }: { searchParams?: Promise<{ type?: string }> }) {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        // Fetch courses with lessons count using new table structure
        const { data: coursesData, error: coursesError } = await supabase
          .from('myuni_kurum_courses')
          .select(`
            *,
            myuni_kurum_course_lessons_user!inner(
              id,
              order_index,
        ...
        // Process courses data to add computed fields from new table structure
        const processedCourses = (coursesData || []).map(course => {
          const courseLessons = course.myuni_kurum_course_lessons_user || [];
          ...
          courseLessons.forEach((courseLesson: { 
            ...
            if (lessonData) {
              if (lessonData.duration_minutes) {
                totalDuration += lessonData.duration_minutes;
              }
          ...
          return {
            ...course,
            sections_count: 1, // For now, consider each course as one section
            lessons_count: lessonsCount,
            total_videos: videoLessonsCount,
  ...
  const handleEditCourse = (course: Course) => {
    ...
    window.location.href = `/${locale}/lms-2/edit/${course.id}`;
  };
