...
export default function InternshipAccessPage({
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/internship/access');
    ...
    async (value: string) => {
      ...
      try {
        const res = await fetch(
          `/api/internship/access/search?q=${encodeURIComponent(trimmed)}`
        );
        const data = await res.json();
  ...
  const handleRevoke = async (clerkUserId: string) => {
    if (!confirm(tr ? 'Erişimi kaldırmak istediğinize emin misiniz?' : 'Revoke access?')) return;
    await fetch(`/api/internship/access?clerkUserId=${clerkUserId}`, { method: 'DELETE' });
    loadMembers();
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <section className="bg-white dark:bg-neutral-800 rounded-xl border p-6 mb-6 shadow-sm">
          <div ref={containerRef} className="relative">
            ...
            {showDropdown && !searching && query.length >= 2 && results.length === 0 && !searchError && (
