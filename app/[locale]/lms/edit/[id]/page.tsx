    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const processedSections: CourseSection[] = (courseData.myuni_course_sections || [])
          .map((section: CourseSection & { myuni_course_lessons?: Array<CourseLesson & { myuni_videos?: CourseVideo[]; myuni_notes?: unknown[]; myuni_quicks?: unknown[] }> }) => ({
            ...section,
            lessons: (section.myuni_course_lessons || [])
              .map((lesson: CourseLesson & { myuni_videos?: CourseVideo[]; myuni_notes?: unknown[]; myuni_quicks?: unknown[] }) => ({
                ...lesson,
                videos: lesson.myuni_videos || [],
                notes: lesson.myuni_notes || [],
                quizzes: lesson.myuni_quicks || []
              }))
...
const CourseContentManager = ({ 
  ...
  // Handle quiz upload
  const handleQuizUploaded = async (uploadedQuiz: CourseQuiz) => {
    // Update lesson whenever quiz is updated/created
    if (selectedLessonForModule) {
      try {
        ...
        // If quiz has time_limit, also update duration_minutes
        if (uploadedQuiz.config?.time_limit) {
          updateData.duration_minutes = Math.round(uploadedQuiz.config.time_limit / 60);
        }
    ...
    // Update local state with both quiz and lesson data
      ...
      lessons: s.lessons.map(l => 
        l.id === selectedLessonForModule 
          ? { 
              ...l, 
              quizzes: editingQuiz 
                ? (l.quizzes || []).map(q => q.id === editingQuiz.id ? uploadedQuiz : q) // Update existing quiz
                : [...(l.quizzes || []), uploadedQuiz], // Add new quiz
              // Update lesson duration if quiz has time_limit
              ...(uploadedQuiz.config?.time_limit && {
                duration_minutes: Math.round(uploadedQuiz.config.time_limit / 60),
                updated_at: new Date().toISOString()
              })
            }
          : l
                              ...
                              {moduleInfo.moduleCount === 0 ? (
                                ...
                                <div>
                                  ...
                                  {lesson.videos && lesson.videos.map((video, videoIndex) => (
                                    <div key={`${lesson.id}-video-${video.id}-${videoIndex}`} className="flex items-center justify-between p-2 bg-white dark:bg-neutral-800 rounded border border-neutral-200 dark:border-neutral-700 mb-2">
                                      <div className="flex items-center space-x-3">
                                  ...
                                  {lesson.notes && lesson.notes.map((note, noteIndex) => (
                                    <div key={`${lesson.id}-note-${note.id}-${noteIndex}`} className="flex items-center justify-between p-2 bg-white dark:bg-neutral-800 rounded border border-neutral-200 dark:border-neutral-700 mb-2">
                                      <div className="flex items-center space-x-3">
                                  ...
                                  {lesson.quizzes && lesson.quizzes.map((quiz, quizIndex) => (
                                    <div key={`${lesson.id}-quiz-${quiz.id}-${quizIndex}`} className="flex items-center justify-between p-2 bg-white dark:bg-neutral-800 rounded border border-neutral-200 dark:border-neutral-700 mb-2">
                                      <div className="flex items-center space-x-3">
                                        ...
                                        {quiz.config?.time_limit && (
                                          <span className="text-xs bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 px-2 py-1 rounded">
                                            {Math.floor(quiz.config.time_limit / 60)}dk
                                          </span>
                                        )}
                                      </div>
                                      <div className="flex items-center space-x-1">
