...
export default function InfluencerLayout({ children, params }: DashboardLayoutProps) {
  ...
  // Check influencer module access
  useEffect(() => {
    const checkInfluencerAccess = () => {
      if (loading) return;
      
      if (error) {
        setHasInfluencerAccess(false);
        return;
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-neutral-100 dark:from-neutral-900 dark:to-neutral-800 flex items-center justify-center py-12">
        <div className="max-w-md mx-auto text-center px-6">
          ...
          <p className="text-neutral-600 dark:text-neutral-400 mb-6 leading-relaxed">
            {locale === 'tr' 
              ? 'Influencer modülüne erişim yetkiniz bulunmamaktadır. Erişim için lütfen yöneticinizle iletişime geçin.'
              : 'You do not have access to the Influencer module. Please contact your administrator for access.'
            }
          </p>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg p-4 mb-6">
            ...
            <div className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
              {locale === 'tr' 
                ? 'Satış performansı ve kampanya yönetimi'
                : 'Sales performance and campaign management'
              }
            </div>
          </div>
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-8 text-xs text-neutral-500 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-800 p-3 rounded">
              ...
              <div>Has influencer access: {hasInfluencerAccess?.toString()}</div>
